...
void draw() {
  ...
  println("counter avant suite de ifs = ", tweetCounter);
  //println("new = ", newTweet, "    old = ", oldTweet);
  ///// SECOND STEP : DISPLAY DATAS ///////
  if (frameCounter % 20 == 0 ) {
    if (tweetCounter < tweets.size() - 1 && tweetCounter < diffTweets ) { // result : 0 < -1 lorsqu'aucun tweet n'a encore été chargé
      println("encooooooooooooooooooremieiiiiiiiiiieeeeeeeeeeeeeeeeeeeeeuuuuuuuuuuuuuuuuuuuxxxxxxxxxxxxxxxxxxxx");
      println("encooooooooooooooooooremieiiiiiiiiiieeeeeeeeeeeeeeeeeeeeeuuuuuuuuuuuuuuuuuuuxxxxxxxxxxxxxxxxxxxx");
      println("encooooooooooooooooooremieiiiiiiiiiieeeeeeeeeeeeeeeeeeeeeuuuuuuuuuuuuuuuuuuuxxxxxxxxxxxxxxxxxxxx");
      println("encooooooooooooooooooremieiiiiiiiiiieeeeeeeeeeeeeeeeeeeeeuuuuuuuuuuuuuuuuuuuxxxxxxxxxxxxxxxxxxxx");
      println("encooooooooooooooooooremieiiiiiiiiiieeeeeeeeeeeeeeeeeeeeeuuuuuuuuuuuuuuuuuuuxxxxxxxxxxxxxxxxxxxx");
  ...
  if (frameCounter % 500 == 0) {
    ...
    try {
      ...
      for (Status status : result.getTweets()) {
        ...
        newTweet = status.getCreatedAt() ;
        float newDate = newTweet.getHours()*3600 + newTweet.getMinutes() * 60 + newTweet.getSeconds() ;
        if (newDate > oldDate 
        ) {
          diffTweets ++ ;
          oldDate = newDate ;
        ...
        energie = map (user.length(), 10, 140, 5, 30) ;  //8000 plus grande valeur observée sur plsueirus longueds observations
